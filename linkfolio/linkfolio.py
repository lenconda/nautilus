...
class LinkFolio:
    ...
    def filter_links(self, prefixed_url):
        with requests.get(prefixed_url, timeout = TIMEOUT) as html_response:
    ...
    def resolve_links(self, item):
        ...
        if url_resolver.check_if_url() \
            ...
            if prefixed_url not in self.result_urls:
                self.filter_links(prefixed_url)
